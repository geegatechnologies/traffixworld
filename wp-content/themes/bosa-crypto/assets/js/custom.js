...
*/
function bosa_crypto_slickNavHeight (){
...
*/
function bosa_crypto_wpMenuAccordion( selector ){
...
function bosa_crypto_fixed_header(){
  $notificationHight = $( '.notification-bar' ).height();
  $logo_selector = document.getElementById( 'headerLogo' );
  var width = $( window ).width();

  if ( $logo_selector && BOSACRYPTO.fixed_nav && BOSACRYPTO.fixed_header_logo ) { 
    if ( $mastheadHeight < $(window).scrollTop()){
      if ( BOSACRYPTO.separate_logo == '' ){
        if ( BOSACRYPTO.the_custom_logo !== '' ){
          $logo_selector.src = BOSACRYPTO.the_custom_logo;
        }
      }else{
        $( '.site-header .site-branding img' ).css( 'display' , 'block' );
        if( !BOSACRYPTO.mobile_fixed_nav_off || width >= 782 ){
          $logo_selector.src = BOSACRYPTO.separate_logo;
        }
      }
    }else{
      if ( BOSACRYPTO.header_two_logo !== '' && ( BOSACRYPTO.is_front_page || BOSACRYPTO.overlay_post || BOSACRYPTO.overlay_page ) && BOSACRYPTO.is_header_two ){
         $logo_selector.src = BOSACRYPTO.header_two_logo;
      }else if ( BOSACRYPTO.the_custom_logo !== '' ) {
          $logo_selector.src = BOSACRYPTO.the_custom_logo;
      }else if ( BOSACRYPTO.separate_logo !== '' ){
        $( '.site-header .site-branding img' ).css( 'display' , 'none' );
      }
    }
  }
  if ( $mastheadHeight > $( window ).scrollTop() || $( window ).scrollTop() == 0 ) {
      if ( BOSACRYPTO.fixed_nav && $( '#masthead.site-header' ).hasClass( 'sticky-header' )){
          $( '#masthead.site-header' ).removeClass( 'sticky-header' );
          // Fixed header in admin bar
          if( BOSACRYPTO.is_admin_bar_showing && width >= 782 ){
            $( '.fixed-header' ).css( 'marginTop', 0 );
          }
          if( BOSACRYPTO.is_admin_bar_showing && width <= 781 ){
            $( '.fixed-header' ).css( 'marginTop', 0 );
          }
      }
  }else if ( BOSACRYPTO.fixed_nav && !$( '#masthead.site-header' ).hasClass( 'sticky-header' )){
    if( !BOSACRYPTO.mobile_fixed_nav_off || width >= 782 ){
      ...
      if( BOSACRYPTO.is_admin_bar_showing && width <= 600 ){
        $( '.fixed-header' ).css( 'marginTop', 0 );
      }
      if( BOSACRYPTO.mobile_fixed_nav_off && width <= 781 ){
        $( '.fixed-header' ).css( 'marginTop', 0 );
    }
  }
}
...
*/
function bosa_crypto_header_two_postion() {
  var width = $( window ).width();

  if ( BOSACRYPTO.is_header_two ) {
    if( BOSACRYPTO.is_admin_bar_showing && width >= 782 ){
...
jQuery( document ).ready( function() {
  bosa_crypto_slickNavHeight();
  bosa_crypto_wpMenuAccordion( '#offcanvas-menu' );
  bosa_crypto_header_two_postion();
  ...
  */
  $( '.header-image-slider' ).slick({
      ...
      fade: BOSACRYPTO.header_image_slider.fade,
      speed: parseInt( BOSACRYPTO.header_image_slider.fadeControl ),
      cssEase: 'linear',
      autoplay: BOSACRYPTO.header_image_slider.autoplay,
      autoplaySpeed: BOSACRYPTO.header_image_slider.autoplaySpeed,
      infinite: true,
  ...
  */
  $( '#primary-menu' ).slicknav({
      ...
      label: BOSACRYPTO.responsive_header_menu_text,
  ...
  */
  $( '.main-slider' ).slick({
      ...
      arrows: true,
      adaptiveHeight: false,
      fade: BOSACRYPTO.main_slider.fade,
      speed: parseInt( BOSACRYPTO.main_slider.fadeControl ),
      cssEase: 'linear',
      autoplay: BOSACRYPTO.main_slider.autoplay,
      autoplaySpeed: BOSACRYPTO.main_slider.autoplaySpeed,
      infinite: true,
  ...
  $( '.highlight-post-slider' ).slick({
      arrows: true,
      dots: true,
      slidesToShow: BOSACRYPTO.home_highlight_posts.slidesToShow,
      slidesToScroll: 1,
      adaptiveHeight: false,
      autoplay: BOSACRYPTO.home_highlight_posts.autoplay,
      autoplaySpeed: BOSACRYPTO.home_highlight_posts.autoplaySpeed,
      infinite: false,
      rows: 0,
...
jQuery( window ).on( 'load', function(){
  ...
  * Back to top
  */
  if( BOSACRYPTO.enable_scroll_top == true && $(window).scrollTop() > 200 ){
...
jQuery( window ).on( 'scroll', function(){
  ...
  * Back to top
  */
  if( BOSACRYPTO.disable_scroll_top == false && $(window).scrollTop() > 200 ){
    $( '#back-to-top' ).fadeIn( 200 );
  } else{
    $( '#back-to-top' ).fadeOut( 200 );
  }
