 ...
 */
add_action( 'admin_enqueue_scripts', 'bosa_crypto_widgets_backend_enqueue' );
function bosa_crypto_widgets_backend_enqueue()
{
    wp_enqueue_style( 'bosa-crypto-getting-started', get_template_directory_uri() . '/inc/getting-started/getting-started.css' );
    wp_register_script( 'bosa-crypto-custom-widgets', get_template_directory_uri() . '/inc/getting-started/getting-started.js', array( 'jquery' ), true );
    $translation = array(
        'btn_text' => esc_html__( 'Processing...', 'bosa-crypto' ),
        'nonce'    => wp_create_nonce( 'bosa_crypto_demo_import_nonce' ),
        'noncen'    => 'bosa_crypto_demo_import_nonce',
        'adminurl'    => esc_url( admin_url() ),
    );
    wp_localize_script( 'bosa-crypto-custom-widgets', 'bosa_crypto_adi_install', $translation );
    wp_enqueue_media();
    wp_enqueue_script( 'bosa-crypto-custom-widgets' );
}

/**
 * Class to handle notices and Advanced Demo Import
 *
 * Class Bosa_Crypto_Notice_Handler
 */
class Bosa_Crypto_Notice_Handler {
	...
	public function __construct() {
        ...
        add_action( 'admin_notices', array( $this, 'bosa_crypto_theme_info_welcome_admin_notice' ), 3 );
        add_action( 'wp_ajax_bosa_crypto_getting_started', array( $this, 'bosa_crypto_getting_started' ) );
    }

    /**
     * Display an admin notice linking to the about page
     */
    public function bosa_crypto_theme_info_welcome_admin_notice() {
        $theme = wp_get_theme();
        if ( is_admin() && !get_user_meta( get_current_user_id(), 'gs_notice_dismissed' ) ){
            ...
            if ( is_plugin_active( 'advanced-import/advanced-import.php' ) && is_plugin_active( 'keon-toolset/keon-toolset.php' ) && is_plugin_active( 'kirki/kirki.php' ) && is_plugin_active( 'elementor/elementor.php' ) && is_plugin_active( 'breadcrumb-navxt/breadcrumb-navxt.php' ) && is_plugin_active( 'contact-form-7/wp-contact-form-7.php' ) && is_plugin_active( 'elementskit-lite/elementskit-lite.php' ) && is_plugin_active( 'bosa-elementor-for-woocommerce/bosa-elementor-for-woocommerce.php' ) ){
                echo ('<div class="fs-notice success"><p>'.esc_html__( 'Plugins added successfully. You may dismiss this notice now.', 'bosa-crypto' ).'</p></div>');
            }else{
            /* translators: %s - Theme name*/
            echo ( '<p><strong>' . sprintf( esc_html__( 'Welcome! Thank you for choosing %1$s. To fully take advantage of the best our theme can offer, get started.', 'bosa-crypto' ), esc_attr( $theme->get( 'Name' ) ) ) . '</strong></p><p class="plugin-notice">'.esc_html__( 'Clicking on get started will install and activate Bosa Elementor Addons for WooCommerce, Kirki, Advanced Import, Keon Toolset, Elementor, ElementsKit Lite, Breadcrumb NavXT and Contact Form 7.', 'bosa-crypto' ).'</p><p><a href="#" class="bosa-crypto-install-plugins button button-primary">' . sprintf( esc_html__( 'Get started with %s','bosa-crypto' ), esc_attr( $theme->get( 'Name' ) ) ) . '</a></p>' );
            }
	 ...
	 * return void
	 */
	public function bosa_crypto_getting_started() {

		check_ajax_referer( 'bosa_crypto_demo_import_nonce', 'security' );
        ...
        if ( ! current_user_can( 'install_plugins' ) ) {
            $status['errorMessage'] = __( 'Sorry, you are not allowed to install plugins on this site.', 'bosa-crypto' );
        ...
        if ( is_wp_error( $result ) ) {
            ...
            $status['errorMessage'] = __( 'Unable to connect to the filesystem. Please confirm your credentials.', 'bosa-crypto' );
...
new Bosa_Crypto_Notice_Handler;